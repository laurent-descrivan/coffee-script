...
/* TFLAGG2 */(repl) {
    ...
    multiline = {
      enabled: false,
      initialPrompt: repl.prompt.replace(/^[^> ]*/, function
/* TFLAGG2 */(x) {
        return x.replace(/./g, '-');
      }),
      prompt: repl.prompt.replace(/^[^> ]*>?/, function
/* TFLAGG2 */(x) {
        return x.replace(/./g, '.');
      }),
      buffer: ''
...
/* TFLAGG2 */(cmd) {
    ...
    return inputStream.on('keypress', function
/* TFLAGG2 */(char, key) {
...
/* TFLAGG2 */(opts) {
      ...
      _ref1 = process.versions.node.split('.').map(function
/* TFLAGG2 */(n) {
        return parseInt(n);
